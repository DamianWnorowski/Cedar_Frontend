...
class HomePage extends Component {
    ...
    componentDidMount(){
        ...
        axios.get(`http://localhost:8080/api/newtvtonight`)
            .then(res => {
                if(res.data.length){
                    const newTV = tvTable(res.data);
                    this.setState({newTV, renderNewTVTonight: true})
                }
        })
    }
                ...
                <Container style={{backgroundColor:'black', color:'white', padding:'2em', marginTop:'-1em'}}>
                    <Grid>
                        ...
                        <Grid.Column width={6}>
                            ...
                            {(this.state.renderNewTVTonight)?<MediaTable gridSize={12} displayInfo={this.state.newTV} numShow={6} nameHeader={<div>New tv<span style={{color:'white'}}>tonight</span></div>}/> : <div />}
                        </Grid.Column>
                    </Grid>
                </Container>
            </div>
